...
**/
package com.project_study.my.ocr_frs.ocr;
...
public class HWOcrClientAKSK {
	 ...
	 * */
	public HttpResponse requestOcrServiceBase64(String uri, Object imgPath, JSONObject params) throws IOException {
		...
		try {
			if (imgPath.toString().indexOf("http://") != -1 || imgPath.toString().indexOf("https://") != -1) {
				params.put("url", imgPath);
			} else if (imgPath instanceof MultipartFile) {
				byte[] fileData = ((MultipartFile) imgPath).getBytes();
				String fileBase64Str = Base64.encodeBase64String(fileData);
				params.put("image", fileBase64Str);
			} else {
				byte[] fileData = FileUtils.readFileToByteArray(new File(imgPath.toString()));
				String fileBase64Str = Base64.encodeBase64String(fileData);
				params.put("image", fileBase64Str);
			}
